const express = require('express');
const router = express.Router();
const { body } = require('express-validator');
const rentingController = require('../controllers/rentingController');
const { isAuthenticated } = require('../middlewares/auth');
const { uploadProduct } = require('../middlewares/upload');

// Renting page
router.get('/', rentingController.renderRenting);

// Rent product form page
router.get('/rent', isAuthenticated, rentingController.renderRentForm);

// Rent product handle
router.post('/rent', isAuthenticated, 
  uploadProduct.array('images', 6), // Allow up to 6 images (1 thumbnail + 5 regular)
  [
    body('name').notEmpty().withMessage('Name is required'),
    body('description').notEmpty().withMessage('Description is required'),
    body('price').isNumeric().withMessage('Price must be a number'),
    body('place').notEmpty().withMessage('Place is required'),
    body('category').notEmpty().withMessage('Category is required'),
    body('whatsappNumber').notEmpty().withMessage('WhatsApp number is required'),
    body('callNumber').notEmpty().withMessage('Call number is required')
  ],
  rentingController.rentProduct
);

module.exports = router;